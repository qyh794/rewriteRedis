...
func (client *Client) Send(args [][]byte) resp.Reply {
	...
	if timeout {
		return reply.NewStanderErrReply("server time out")
	}
	if request.err != nil {
		return reply.NewStanderErrReply("request failed")
	}
...
func (client *Client) handleRead() error {
	...
	for payload := range ch {
		if payload.Err != nil {
			client.finishRequest(reply.NewStanderErrReply(payload.Err.Error()))
			continue
		}
